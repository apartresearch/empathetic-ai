...
```{r}
pacman::p_load(tidyverse, readr, stringr, ggplot2, ggpubr)
...
plot2 %>% 
    ## CLEANING DATA ##
    filter(!is.na(Hurricane_Type)) %>% #Filtering out Hurricane_Type that are NA
    mutate(Hurricane_Type = ordered(Hurricane_Type, #Ordering it as a factor
          levels = c("Tropical Depression", "Tropical Storm", "Hurricane", "Major Hurricane"))) %>%
  
    ## INITIALIZING PLOT ##
    ggplot(., aes(x=as.factor(MONTH), y=avg)) + #Setting aesthetics with month as a factor
    geom_bar(stat="identity",    #Using stat = 'identity' so heights of the bars to represent values in the data
             width=0.7,          #Setting width of bars
             fill="steelblue") + #Setting color
    geom_errorbar(aes(ymin=avg-sd, ymax=avg+sd), #Initializing error bars
              width = .2) +       #Setting width of error bars
    theme_minimal() +             #Setting theme 
    xlab("Month") +               #Changing x label
    ylab("Number of Hurricanes") +#Changing y label
    facet_wrap(~Hurricane_Type) + #Facetting by hurricane type
    ggtitle("Average Number of Hurricanes Per Type and Month with Error Bars") 
```
