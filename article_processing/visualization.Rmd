...
ggsave("../figures/3_study_definition.png", p2, device="png", units="px", width=1000, height=880)
```

## Approach / study design and definition implemented / empathy domain
```{r}
df <- gsheet2tbl("https://docs.google.com/spreadsheets/d/14ZDSEwc9PZZRNYIldqQoPm34uWidiglJ5RB3bn5YnQQ/edit#gid=1156551034") %>% 
  drop_na(Domain)
...
ggsave("../figures/4_definition_domain.png", p1, units="px", width=1000, height=800)
ggsave("../figures/4_study_approach.png", p2, device="png", units="px", width=1000, height=880)
```

## Human and robot studies
```{r}
df <- gsheet2tbl("https://docs.google.com/spreadsheets/d/14ZDSEwc9PZZRNYIldqQoPm34uWidiglJ5RB3bn5YnQQ/edit#gid=1156551034") %>% 
  mutate(hr = if_else(str_to_lower(`Studying human's  or robot's response?`) == "r", "Robot", "Human"))
...
p1 <- df %>% 
  ...
  ungroup() %>% 
  mutate(
    n_s = if_else(is.na(study_design), as.integer(0), n_s),
    study_design = if_else(is.na(study_design), "Not specified", study_design),
    study_design = fct_reorder(study_design, n_s)
  ) %>% 
  ggplot() +
  aes(x=n, y=study_design, fill=hr) +
  geom_col() +
  labs(y=NULL, x="Number of studies", fill="Study focus") +
  theme_classic() +
  coord_cartesian(expand=F, xlim=c(0,50)) + 
  scale_fill_tableau() +
  theme(legend.position = c(0.7, 0.4))

ggsave("../figures/5_human_robot.png", p1, units="px", width=1000, height=700)
```
