...
Emp_dom_Per_Country <- articles_coded %>%
      ...
      GB = paste0(str_count(country, "GB"))) %>% 
      pivot_longer(., 4:41) %>% 
      mutate(value = as.numeric(value)) %>% view()
...
### Getting stats from measurement methods

```{r}
MM_Stat <- articles_coded %>%
    summarise(
    year = year,
    study_design = study_design,
    robot_human,
...
### First getting approach statistics

```{r}
app_stat <- articles_coded %>%
    summarise(
    year = year,
    approach = approach,
    robot_human,
...
plot_dat_4 = 
    ...
    "Machine Learning" = paste0(str_count(study_design, "Machine learning")),
    Visual = paste0(str_count(study_design, "Visual")),
    "Behavior model" = paste0(str_count(study_design, "Behaviour model"))) %>% 
    pivot_longer(., 4:9) %>% 
    mutate(study_design = ifelse(value == 1, name, NA)) %>% 
    drop_na() %>%
    select(1:3, 6) %>% 
...
theme_update <- theme_minimal() +
  theme(
    ...
    strip.text.x = element_text(hjust = 0, margin=margin(l=0, b=1)),
  )

p1 <- p1 + theme_update + theme(plot.subtitle = element_text(hjust=-0.38, vjust=-22.5))
p2 <- p2 + theme_update + theme(plot.subtitle = element_text(hjust=-1.04, vjust=-26))
p3 <- p3 + theme_update + theme(plot.subtitle = element_text(hjust=-0.55, vjust=-23))
p4 <- p4 + theme_update + theme(plot.subtitle = element_text(hjust=-0.93, vjust=-26))
...
```{r}
emp_app_long <- articles_coded %>% view()
    summarise(
    year = year,
    country = country,
    approach = approach,
...
p <- articles_coded %>%
  ggplot( aes(x=Citations)) +
    geom_histogram(fill = tableau_pal[1], alpha=0.3, color=tableau_pal[1], position = 'identity', binwidth = 20) +
    theme_eai +
  coord_cartesian(expand=F) +
  labs(y=NULL, x="Citations (median = 3)") +
  geom_curve(
    data = arrows, aes(x = x1, y = y1, xend = x2, yend = y2),
    arrow = arrow(length = unit(0.08, "cm")), size = 0.5,
    color = "gray20", curvature = -0.3) +
  annotate("text", x = 200, y = 75, label = "\"A New Chatbot for\nCustomer Service\non Social Media\"\n299 citations", size = 3)

p
```

### Researchers per article plot

```{r}
getwd()
researcher_data <- read_delim("../articles/researcher_data.csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE)

data_split <- str_split_fixed(researcher_data$Author_New, ';', n = 12)
data_split <- as.data.frame(data_split)
data_split$year = researcher_data$Publication_Year

data_split2 = data_split %>% 
      pivot_longer(1:12) %>%
      select(1,3) %>% 
      mutate(value2 = ifelse(value == "", NA, value)) %>% 
      drop_na()

### Removing white space in front of each name

data_trim = data.frame()

for (row in 1:length(data_split2$year)) {
    data_trim[row, 1] = trimws(data_split2$value2[row])
      }

data_split2$Name_Clean <- data_trim$V1
```

### Articles per Resarcher plot

```{r, fig.height=1, fig.width=1}
APP <- data_split2 %>% 
  group_by(Name_Clean) %>% 
  summarise(Count_Researcher = n()) %>%
  ggplot(aes(x = Count_Researcher)) +
  geom_bar(fill= tableau_pal[1], alpha=0.4, color=tableau_pal[1], width=1) +
  theme_light() +
  xlab("Articles Per Researcher") +
  ylab(NULL) +
  coord_cartesian(expand=F, clip="off") +
  theme_eai


### Making arrows for the plot
...
### Adding arrows to plot and annotating

APP + geom_curve(
    data = arrows, aes(x = x1, y = y1, xend = x2, yend = y2),
    arrow = arrow(length = unit(0.08, "inch")), size = 0.5,
    color = "gray20", curvature = -0.3) +
      annotate("text", x = 3.2, y = 400, label = "Tomio Watanabe", size = 3) +
      annotate("text", x = 3.3, y = 350, label = "Hiroshi Ishiguro", size = 3) 
...
```{r, fig.height=1, fig.width=1}
RPA_Plot <- researcher_data %>% 
            mutate(count_researchers = str_count(.$Author_New, pattern = ";")+ 1 ) %>% 
            ggplot(aes(x = count_researchers)) +
            geom_bar(fill = tableau_pal[1], alpha=0.4, color=tableau_pal[1], width=1) +
            theme_light() +
            xlab("Researchers Per Article") +
            ylab(NULL) +
...
```{r}
researcher_data1 <- researcher_data %>% 
            mutate(count_researchers = str_count(.$Author_New, pattern = ";")+ 1 ) 


researcher_data1 %>% 
            summarise(median(count_researchers))
```


### Citations per article plot

```{r}

median_citations <- median(articles_coded$Citations, na.rm = T)

p <- articles_coded %>%
    ggplot( aes(x = Citations)) +
    geom_density(fill = "#597787", color = "#597787") +
    theme_light() +
    xlab("Citations per article") +
    ylab("Density") + 
    xlim(0,100) + 
    geom_vline(xintercept=median_citations, size=1, color="black")+
    geom_text(aes(x=median_citations+7, label=paste0("Median: ",median_citations), y=0.08))
...
```{r}
p_cit <- articles_coded %>%
    group_by(year) %>% 
    summarise(Citations_Count = mean(Citations, na.rm = T)) %>% 
    ggplot( aes(x = year, y = Citations_Count)) +
...
```{r, fig.height=1, fig.width=1.8}
topic_counts <- articles_coded %>% 
                   group_by(Topic, year) %>% 
                   summarise(count = n())

theme_update_2 <- theme_minimal() +
  theme_eai

ggplot(topic_counts, aes(fill=Topic, color=Topic, y=count, x=year)) + 
    geom_bar(position="stack", stat="identity", alpha=0.4, width=1) + 
    #coord_flip() +
    scale_fill_tableau() + 
  scale_color_tableau() +
    theme_update_2 +
...
## Clustering visualization Summing to 100%

```{r}
topic_counts <- articles_coded %>% 
                   group_by(Topic, year) %>% 
                   summarise(count = n())
topic_PY_counts <- articles_coded %>% 
                   group_by(year) %>% 
                   summarise(count_year = n())
                

topic_tot <- left_join(topic_counts, topic_PY_counts)

topic_tot <- topic_tot %>% 
             mutate(percentage = (count/count_year)*100)
...
ggplot(topic_tot, aes(fill=Topic, y=percentage, x=year)) + 
